      ...
      <div className="space-y-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xl text-gray-600 mt-1">{className} {section}</p>
            <p className="text-lg text-gray-500 mt-1">Manage syllabus content and track progress</p>
          </div>
          <Link
                ...
                <CollapsibleTrigger className="w-full p-6 flex items-center justify-between hover:bg-gray-50 transition-colors">
                  <div className="flex items-center gap-6">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-lg font-semibold text-gray-700 min-w-[50px]">{chapter.progress}%</span>
                    </div>
                  </div>
                  ...
                  <div className="border-t border-gray-200 p-6">
                    <Tabs defaultValue="topics" className="w-full">
                      <TabsList className="grid w-full grid-cols-3 mb-6">
                        <TabsTrigger value="topics" className="text-lg py-3">Topics</TabsTrigger>
                        <TabsTrigger value="lessonplan" className="text-lg py-3">Lesson Plan</TabsTrigger>
                        <TabsTrigger value="prerequisites" className="text-lg py-3">Prerequisites</TabsTrigger>
                      </TabsList>
                        ...
                        <div className="space-y-4">
                          ...
                          {chapter.topics.map((topic, index) => (
                            <div key={index} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-200 hover:bg-gray-100 transition-colors">
                              <div className="flex items-center gap-4 flex-1">
                                <span className="text-lg font-semibold text-blue-600 bg-blue-100 rounded-full w-8 h-8 flex items-center justify-center">
                                  {index + 1}
                                </span>
                                {editingTopic?.chapterId === chapter.id && editingTopic?.topicIndex === index ? (
                                  <div className="flex items-center gap-3 flex-1">
                                    <Input
                                      ...
                                      className="flex-1 text-lg py-3 border-2 border-blue-300 focus:border-blue-500"
                                      onKeyDown={(e) => {
                                        if (e.key === 'Enter') {
                          ...
                          {addingTopic === chapter.id && (
                            <div className="flex items-center gap-3 p-4 bg-blue-50 rounded-lg border-2 border-blue-200">
                              <span className="text-lg font-semibold text-blue-600 bg-blue-200 rounded-full w-8 h-8 flex items-center justify-center">
                            ...
                            chapter.lessonPlans.map((lessonPlan) => (
                              <div key={lessonPlan.id} className="p-5 bg-gray-50 rounded-lg border border-gray-200 hover:shadow-md transition-shadow">
                                <div className="flex items-center justify-between mb-3">
                                  <h4 className="text-lg font-semibold text-gray-800">{lessonPlan.title}</h4>
                                  <div className="flex items-center gap-3">
                                ...
                                <p className="text-base text-gray-600">{lessonPlan.description}</p>
                              </div>
                            ))
                          ) : (
                            <div className="text-center py-12 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
                              <FileText className="w-16 h-16 mx-auto mb-4 text-gray-400" />
                              <h4 className="text-xl font-medium text-gray-600 mb-2">No lesson plans created yet</h4>
                              <p className="text-lg text-gray-500 mb-4">Create your first lesson plan to get started</p>
                                ...
                                {editingPrerequisite?.chapterId === chapter.id && editingPrerequisite?.prereqIndex === index ? (
                                  <div className="flex items-center gap-3 flex-1">
                                    <Input
                                      ...
                                      className="flex-1 text-lg py-3 border-2 border-yellow-300 focus:border-yellow-500"
                                      onKeyDown={(e) => {
                                        if (e.key === 'Enter') {
                                  ...
                                  <span className="text-lg text-gray-800">{prerequisite}</span>
                                )}
                              </div>
                              {!editingPrerequisite && (
                                <Button
                          ...
                          {addingPrerequisite === chapter.id && (
                            <div className="flex items-center gap-3 p-4 bg-yellow-100 rounded-lg border-2 border-yellow-300">
                              ...
                              <Input
                                ...
                                className="flex-1 text-lg py-3 border-2 border-yellow-400 focus:border-yellow-500"
                                onKeyDown={(e) => {
                                  if (e.key === 'Enter') {
